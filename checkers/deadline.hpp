#ifndef _CHECKERS_DEADLINE_H_
#define _CHECKERS_DEADLINE_H_
...
public:
    ...
    explicit Deadline(int64_t pTime=-1)
        :   mTime(pTime)
    {
    }
    ...
    ///returns a Deadline object representing the current time
    static Deadline now()
    {
        struct timeval lTime;
        gettimeofday(&lTime,NULL);
        return Deadline(int64_t(lTime.tv_sec)*1000000L+lTime.tv_usec);
    }
    ...
    Deadline operator+(int64_t pDelta) const
    {
    ...
    int64_t operator-(const Deadline &pRH) const
    {
