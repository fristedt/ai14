...
std::string GameState::toString(int pPlayer) const
{
    ...
    ss << board_top;
    ss << "  1 " << board_left; for(int c=0;c<8;++c) ss << cell_text[at(0,c)]; ss << board_right << " 4\n";
    ss << "  5 " << board_left; for(int c=0;c<8;++c) ss << cell_text[at(1,c)]; ss << board_right << " 8\n";
    ss << "  9 " << board_left; for(int c=0;c<8;++c) ss << cell_text[at(2,c)]; ss << board_right << " 12     Last move: " + mLastMove.toString() + (is_winner ? " (WOHO! I WON!)\n" : "\n");
    ss << " 13 " << board_left; for(int c=0;c<8;++c) ss << cell_text[at(3,c)]; ss << board_right << " 16     Next player: " + cell_text[mNextPlayer] + (is_my_turn ? " (My turn)\n" : " (Opponents turn)\n");
    ss << " 17 " << board_left; for(int c=0;c<8;++c) ss << cell_text[at(4,c)]; ss << board_right << " 20     Moves until draw: " << (int)mMovesUntilDraw << "\n";
    ss << " 21 " << board_left; for(int c=0;c<8;++c) ss << cell_text[at(5,c)]; ss << board_right << " 24     Red pieces:   " << red_pieces << "\n";
    ss << " 25 " << board_left; for(int c=0;c<8;++c) ss << cell_text[at(6,c)]; ss << board_right << " 28     White pieces: " << white_pieces << "\n";
    ss << " 29 " << board_left; for(int c=0;c<8;++c) ss << cell_text[at(7,c)]; ss << board_right << " 32\n";
    ss << board_bottom;
